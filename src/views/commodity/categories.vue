...
<div>
    ...
    <el-card>
      ...
      <tree-table class="treeTable" :data="catelist" :columns="columns" :selection-type="false" :expand-type="false" show-index index-text="#" border :show-row-hover="false">
        ...
        <template slot="opt" slot-scope="scope">
          <el-button type="primary" icon="el-icon-edit" size="mini" @click="hqCate(scope.row.cat_id,scope.row.cat_name)">编辑</el-button>
	...
	<el-dialog title="修改分类名称" :visible.sync="dialogVisible" width="30%" >
		<el-form label-width="100px" :rules="rules" ref="xgCate" :model="xgCate" class="demo-dynamic">
			<el-form-item label="分类名称" prop="cateName">
				<el-input v-model="xgCate.cateName"></el-input>
			</el-form-item>
		</el-form>
		<span slot="footer" class="dialog-footer">
			<el-button @click="dialogVisible = false">取 消</el-button>
			<el-button type="primary" @click="putCate">确 定</el-button>
		</span>
	</el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
	...
	//打开分类铭框
	hqCate(cat_id,cat_name){
		console.log(cat_id,cat_name)
		this.dialogVisible = true;
		this.xgCate.cateId=cat_id;
		this.xgCate.cateName=cat_name;
		
	},
	// 编辑分类铭
	putCate(){
		this.$refs.xgCate.validate(async (res) => {
			if (!res) {
				return;
			}
			await this.$http.put(`categories/${this.xgCate.cateId}`, {
				cat_name: this.xgCate.cateName,
			}).then(res => {
				console.log(res)
				let data = res.data;
